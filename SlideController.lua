...
function SlideController:_resolveFlyButtonNodes()
    ...
    if flyButtonRoot and flyButtonRoot.Parent
        and holdButton and holdButton.Parent
        and leftButton and leftButton.Parent
        and rightButton and rightButton.Parent then
        return flyButtonRoot, holdButton, leftButton, rightButton
    end
    ...
    if not (holdButton and holdButton:IsA("GuiButton")) then
        return nil, nil, nil, nil
    end
...
function SlideController:_ensureFlyButtonBindings()
    ...
    local flyButtonRoot, holdButton, leftButton, rightButton = self:_resolveFlyButtonNodes()
    if not (flyButtonRoot and holdButton and leftButton and rightButton) then
        return nil, nil, nil, nil
    end
    ...
    self:_bindFlyButtonState(holdButton, function(isPressed)
        self._flyHoldPressed = isPressed == true
    end)
...
function SlideController:_applyLaunchMomentum(rootPart)
    ...
    if launchMomentumVelocity and self._flyLateralVelocity.Magnitude > MIN_DIRECTION_MAGNITUDE then
        launchMomentumVelocity = launchMomentumVelocity + (self._flyLateralVelocity * airControlConfig.momentumBlend)
    end
...
function SlideController:_startBulletTime(rootPart)
    ...
    local baselineVelocity = rootPart.AssemblyLinearVelocity
    local baselineLaunchMomentumVelocity = flattenVector(baselineVelocity)
    if baselineLaunchMomentumVelocity.Magnitude <= MIN_DIRECTION_MAGNITUDE then
        baselineLaunchMomentumVelocity = self._launchMomentumVelocity
    end

    self._isBulletTimeActive = true
    self._bulletTimeBaselineVelocity = baselineVelocity
    self._bulletTimeBaselineLaunchMomentumVelocity = baselineLaunchMomentumVelocity
    ...
    if baselineLaunchMomentumVelocity and baselineLaunchMomentumVelocity.Magnitude > MIN_DIRECTION_MAGNITUDE then
        self._launchMomentumVelocity = baselineLaunchMomentumVelocity.Unit * fallSpeed
    end
end
...
function SlideController:_stopBulletTime(rootPart, shouldRestoreVelocity)
    ...
    local baselineVelocity = self._bulletTimeBaselineVelocity
    ...
    if shouldRestoreVelocity ~= true or not (rootPart and rootPart.Parent) then
        return
    end

    if baselineVelocity then
        rootPart.AssemblyLinearVelocity = baselineVelocity
    end
...
function SlideController:_updateLaunchFlightControls(humanoid, rootPart, deltaTime)
    ...
    if self:_isFlyHoldActive() then
        ...
        self:_stopBulletTime(rootPart, true)
    end
