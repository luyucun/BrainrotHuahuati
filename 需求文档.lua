...
2003	3	3	4096000000
2004	4	3	8192000000
2005	5	3	16384000000
2006	6	3	32768000000
2007	7	3	65536000000
2008	8	3	131072000000
2009	9	3	262144000000
2010	10	3	524288000000
...
102	3	20000000	59	3565743801
103	4	600000000	99	3565744240
104	5	9000000000	159	3565744441
...
需求文档V4.1 boss追逐
概述：我们要做一套boss，boss出生在脑红场地中，玩家拿起脑红后boss会追逐玩家，追上后攻击玩家玩家会被击飞

详细规则：
1.boss我们直接使用的是脑红的模型形象，当一个脑红我们设定为boss的时候，我们会给这个脑红配置对应的移动动作（walk）以及攻击动作（attack）
2.我们会给一些可能会刷新脑红的区域，配置对应的boss，我们配置在脑红生成组配置这张表里，更新后的表是这样的：
生成组id	生成位置	同时存在脑红数量上限	是否有boss（1是0否）	boss模型放大倍数	脑红Boss模型	移动动作id	攻击动作id	移动速度（米/秒）
1001	Common1	8	1	5	ReplicatedStorage/Model/Normal/TungSahur	140442378493259 	76166960595723 	1
1002	Uncommon1	8	1	5	ReplicatedStorage/Model/Normal/CappuccinoAssassino	109915074386387 	135688434700674 	1.5
1003	Rare1	8	1	5	ReplicatedStorage/Model/Normal/Matteo	130956934142232 	106750282937778 	2
1004	Epic1	8	1	5	ReplicatedStorage/Model/Normal/BallerinaCappuccina	120282135124834 	124866685818663 	2.5
1005	Legendary1	8	1	5	ReplicatedStorage/Model/Normal/TralaleroTralala	91857463558938 	77157247219120 	3
1006	Mythic1	8	1	5	ReplicatedStorage/Model/Normal/EsokSekolah	115989105041452 	105725803464865 	3.5
1007	Secret1	8	1	5	ReplicatedStorage/Model/Normal/SixSeven	135635622583201 	91617626103661 	4
1008	God1	8	1	5	ReplicatedStorage/Model/Normal/StrawberryElephant	93087862236418 	84102408569698 	4.5
1009	OG1	8	1	5	ReplicatedStorage/Model/Normal/GorilloWatermelondrillo	95458933878415 	132843466875829 	5


以上的表我加了一字段：
1.是否有boss，如果填了是，在游戏创建后需要在这个场中生成一个boss，boss对应的信息都填在后面，如果填的否，那么这个区域没有boss
2.脑红boss模型，这个填的是模型的路径
3.移动动作id和攻击动作id，这俩都是对应boss的id
4.移动速度，这个是米/秒，我填了1.5就代表每秒走1.5米
5.boss模型放大倍数就是这个boss的模型使用后要在原版的基础上整体放大多少倍，让boss看起来很大

boss生成后：需要始终在自己生成的区域内巡逻，不要超出边界，这个巡逻规则由AI帮我设计一套，反正就是比如隔个几秒，就找个目标点走过去，中间有一定几率停下来换目标点或者简单站着愣一下，由AI设计
boss的攻击逻辑是：
1.一旦有玩家拿起了脑红：boss就解除巡逻状态，改成朝玩家移动，并始终追逐玩家
2.当boss与玩家的距离小于X米（这里暂定1米）的时候，boss开始攻击玩家，只要触发了攻击动作，玩家必定被击中
3.玩家被攻击后就是正常的被击飞（类似被棒球棍打），然后丢失当前正在举起来的脑红，然后被举起来的脑红解除被人拿着的逻辑，这个是现成的已经做好的逻辑
4.玩家无法攻击boss
5.玩家拿起脑红后，立刻显示出来StarterGui - Main - Drop
6.玩家点击StarterGui - Main - Drop - DropButton这个按钮，可以把当前拿起的所有脑红放到地上，变成没拿脑红的状态（注意这里不是我背包里已经有的脑红），一旦放下后，boss立刻解除对玩家的追逐，然后进入巡逻状态
7.StarterGui - Main - Drop - Home - CountDownTime是一个文本，用于显示倒计时，精确到小数点后一位，玩家拿起脑红后开始倒计时5秒
8.倒计时结束之前，需要把StarterGui - Main - Drop - Home这个按钮置灰，不能点击，然后当倒计时结束后，把按钮解除置灰，变成可以点击状态，点击按钮后，立刻把玩家传送回home，同时隐藏StarterGui - Main - Drop
9.当boss开始追逐玩家时：需要做个小提醒：StarterGui - Main - Warning是一个警告提醒，需要连续3次：出现隐藏出现隐藏，出现的时候要渐变出现，隐藏的时候要渐变消失，看起来是闪烁的那种效果
10.在玩家拿起脑红，倒计时结束后如果玩家没点击home键回家，又拿起新的脑红，那么需要继续重新开始倒计时，也就是玩家每次拿起新的脑红，都开始重新倒计时

V4.1.1 关于脑红生成的路径修改
生成组id	生成位置	同时存在脑红数量上限	是否有boss（1是0否）	boss模型放大倍数	脑红Boss模型	移动动作id	攻击动作id	移动速度（米/秒）
1001	Land/FloatLand1/Land	8	1	5	ReplicatedStorage/Model/Boss/TungSahur	140442378493259 	76166960595723 	1
1002	Land/FloatLand2/Land	8	1	5	ReplicatedStorage/Model/Boss/CappuccinoAssassino	109915074386387 	135688434700674 	1.5
1003	Land/FloatLand3/Land	8	1	5	ReplicatedStorage/Model/Boss/Matteo	130956934142232 	106750282937778 	2
1004	Land/FloatLand4/Land	8	1	5	ReplicatedStorage/Model/Boss/BallerinaCappuccina	120282135124834 	124866685818663 	2.5
1005	Land/FloatLand5/Land	8	1	5	ReplicatedStorage/Model/Boss/TralaleroTralala	91857463558938 	77157247219120 	3
1006	Land/FloatLand6/Land	8	1	5	ReplicatedStorage/Model/Boss/EsokSekolah	115989105041452 	105725803464865 	3.5
1007	Land/FloatLand7/Land	8	1	5	ReplicatedStorage/Model/Boss/SixSeven	135635622583201 	91617626103661 	4
1008	Land/FloatLand8/Land	8	1	5	ReplicatedStorage/Model/Boss/StrawberryElephant	93087862236418 	84102408569698 	4.5
1009	Land/FloatLand9/Land	8	1	5	ReplicatedStorage/Model/Boss/GorilloWatermelondrillo	95458933878415 	132843466875829 	5

说明：我们主要是对生成位置进行了修改，以1001这个生成组举例，生成位置是Land/FloatLand1/Land，意思是去Workspace下去找Land这个文件夹，按对应路径能找到一个叫Land的Part，在这个Part上生成脑红和boss


V4.1.2 补充关于特殊事件的一些功能
1.当特殊事件发生的时候，需要在玩家的界面上弹出对应的系统提示，具体逻辑是：
    a.StarterGui - Main - EventStart是用来承载事件的提示文本的，里面有多个节点，比如DiamondEvent/HackerEvent这些
    b.DiamondEvent/HackerEvent这些平时是隐藏的，以及EventStart也是隐藏的
    c.在我们之前的逻辑中，也就是“需求文档V3.3.2 特殊事件客户端逻辑显示”这一块中，事件类型有对应的事件文本的名字配置，在EventStart下，也是用的一样的名字
    d.现在做的需求是：当一个事件发生的时候，要把EventStart显示出来，同时把对应名字的EventStart的子节点的文本也显示出来，同时修改其文本：xxx Event Start! 其中xxx就是事件名字
    e.具体出现效果就和现在的其他交互弹出的tips效果一样即可


V4.1.3 滑梯下落交互规则调整
1.废弃“需求文档V3.2.2 减速效果实现”里关于 FlyButton - Hold、空格键减速、子弹时间减速的实际玩法效果；玩家进入下落阶段后，按住空格或 Hold 都不再减速。
2.可操作下落阶段的 FlyButton 只保留 Left / Right / Land 三种交互；FlyButton - Hold 永远不再显示。
3.PC 端 FlyButton - Tips 需要移除 Space / Slowdown 提示，只保留左右修正与快速落地提示；移动端同样不再显示 Hold。
4.JetpackConfig 里的 BulletTimeFallSpeed 当前仅保留数据兼容，不再驱动玩家可见玩法表现或客户端飞行结算；后续如果要彻底删除该字段，需要再单独出一版数据结构调整需求。

V4.2 脑红突变
1.我们需要在脑红生成时，去给脑红补一层突变逻辑，决定本次生成的脑红的稀有度是多少
2.我们的脑红配置表中，每个脑红都配置了不同的稀有度，比如10001/20001/30001这些，本质上是同一个脑红的不同稀有度
3.我们设定一个突变表：
	稀有度	突变权重
	1	70
	2	20
...
在脑红生成时，根据权重生成一个概率，决定本次生成的脑红是什么稀有度，从而决定本次生成的脑红是脑红表中哪个id的脑红
